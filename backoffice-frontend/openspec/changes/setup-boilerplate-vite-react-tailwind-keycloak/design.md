...
O diretorio `backoffice-frontend/` esta vazio (sem codigo). O backend Spring Boot ja esta funcional em `backoffice-backend/` com APIs REST, WebSocket/STOMP e autenticacao via Keycloak. O frontend sera uma SPA React que consome essas APIs.

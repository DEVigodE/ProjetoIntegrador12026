...
SPA React para o backoffice de um sistema de Delivery de Alimentos. Consome exclusivamente as APIs REST e WebSocket do backend Spring Boot (`logdash-backend/`). Uso interno da equipe do restaurante (Admin, Operador, Despachante).
...
# 4. Subir infraestrutura (na pasta logdash-backend/)
cd ../logdash-backend && docker-compose up -d

# 5. Rodar backend (na pasta logdash-backend/)
./mvnw spring-boot:run -Dspring-boot.run.profiles=dev

# 6. Rodar frontend (na pasta logdash-frontend-web/)
cd ../logdash-frontend-web && npm run dev
```
...
Referencia completa da API, arquitetura, e regras de negocio em `../logdash-backend/CLAUDE.md`.

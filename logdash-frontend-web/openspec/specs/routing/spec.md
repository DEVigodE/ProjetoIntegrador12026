...
Roteamento SPA com React Router v7, rotas protegidas por autenticacao e roles.
...
O sistema SHALL ter um componente `RoleGuard` que le as roles do JWT via `keycloak.tokenParsed?.realm_access?.roles`. Se o usuario possuir pelo menos uma das roles exigidas, SHALL renderizar o conteudo filho. Caso contrario, SHALL exibir uma mensagem de "Acesso negado" (nao redirecionar).
